 ...
 * Displays section management with list and add form
 * Features: Section listing, filtering, sorting, add new section
 */
...
const SAMPLE_SECTIONS = [
  ...
  },
  {
    ...
    name: 'B',
    class: 'Class 12',
    classTeacher: 'Mr. Ayush Kumar',
    note: 'Commerce stream',
  },
  {
    id: 7,
...
const ManageSection = () => {
  ...
  useEffect(() => {
    ...
    const fetchSections = async () => {
      setLoading(true);
      try {
        const response = await apiService.get('/academic/sections/', { params: { page_size: 200 } });
        const results = response.data?.results || [];
        if (cancelled) return;
        setSections(results.map((s) => ({
          id: s.id,
          school: '',
          name: s.name,
          class: s.class_name || '—',
          classTeacher: s.section_teacher_name || '—',
          note: s.note || '',
        })));
      } catch (err) {
        if (!cancelled) setError(err.response?.data?.message || 'Failed to load sections');
      } finally {
      ...
      <Paper sx={{ boxShadow: 1 }}>
        ...
        >
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
            <MenuIcon sx={{ color: '#000', fontSize: 28 }} />
                  ...
                  {sections.length === 0 ? (
                    ...
                    sections.map((section, index) => (
                      ...
                      >
                        ...
                        <TableCell>{section.school}</TableCell>
